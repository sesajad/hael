...
class UFWCommandRule(UFWCommand):
    ...
    def parse(self, argv):
        ...
        if len(argv) > 0:
            if argv[0].lower() == "delete" and len(argv) > 1:
                ...
                if insert_pos == "0" or insert_pos == "-1":
                    err_msg = ("Cannot insert rule at position '%s'") % \
                                (insert_pos)
        ...
        if nargs > 1 and (argv.count('in') > 0 or argv.count('out') > 0):
            err_msg = ("Invalid interface clause")
        ...
        if "log" in argv:
            err_msg = ("Option 'log' not allowed here")
            raise UFWError(err_msg)

        if "log-all" in argv:
            err_msg = ("Option 'log-all' not allowed here")
            raise UFWError(err_msg)
        ...
        if nargs == 2:
            ...
            if rule.dapp == "":
                ...
                if not re.match('^\d([0-9,:]*\d+)*$', port):
                    if ',' in port or ':' in port:
                        err_msg = ("Port ranges must be numeric")
                        raise UFWError(err_msg)
                    to_service = port
            ...
            raise UFWError(err_msg)
             ...
             'out' not in argv:
            err_msg = ("Need 'to' or 'from' clause")
            raise UFWError(err_msg)
        else:
            ...
            for arg in argv:
                if i % 2 != 0 and argv[i] not in keys:
                    err_msg = ("Invalid token '%s'") % (argv[i])
                    raise UFWError(err_msg)
                if arg == "proto":
                    if i+1 < nargs:
                        try:
                    ...
                    if i+1 < nargs:
                        ...
                        # checks above, but leave it here in case our parsing
                        # changes
                        err_msg = ("Invalid 'to' clause")
                        raise UFWError(err_msg)
                elif arg == "port" or arg == "app":
                    if i+1 < nargs:
                        if loc == "":
                            err_msg = ("Need 'from' or 'to' with '%s'") % \
                                        (arg)
                            raise UFWError(err_msg)
            ...
            if from_type == "any" and to_type == "any":
                type = "both"
                 ...
                 from_type != to_type:
                err_msg = ("Mixed IP versions for 'from' and 'to'")
                raise UFWError(err_msg)
            elif from_type != "any":
                type = from_type
            elif to_type != "any":
        ...
        if to_service != "" or from_service != "":
            ...
            if to_service != "":
                try:
                    ...
                    # checks above, but leave it here in case our parsing
                    # changes
                    err_msg = ("Could not find protocol")
                    raise UFWError(err_msg)
            if from_service != "":
                if proto == "any" or proto == "":
                    ...
                    if proto == "any" or proto == tmp:
                        ...
                        err_msg = ("Protocol mismatch (from/to)")
                        raise UFWError(err_msg)
            ...
            if rule.protocol == "any":
                ...
                err_msg = ("Protocol mismatch with specified protocol %s") % \
                            (rule.protocol)
                raise UFWError(err_msg)
...
class UFWCommandRouteRule(UFWCommandRule):
    ...
    def parse(self, argv):
        ...
        if 'delete' in argv:
            ...
            if len(argv) > idx:
                try:
                    ...
                    int(argv[idx + 1])
                    err_msg = ("'route delete NUM' unsupported. Use 'delete NUM' instead.")
                    raise UFWError(err_msg)
                except ValueError:
        ...
        if " in on " in s and " out on " in s:
            ...
            err_msg = ("Invalid interface clause for route rule")
            raise UFWError(err_msg)
        else:
            rule_argv = argv
...
class UFWParser:
    ...
    def register_command(self, c):
        ...
        if key in self.commands[c.type]:
            err_msg = ("Command '%s' already exists") % (key)
            raise UFWError(err_msg)
