...
class UFWRule:
    ...
    def set_protocol(self, protocol):
        ...
        if protocol in ufw.util.supported_protocols + ['any']:
            ...
            err_msg = ("Unsupported protocol '%s'") % (protocol)
            raise UFWError(err_msg)
    ...
    def set_dst(self, addr):
        ...
        if tmp != "any" and not ufw.util.valid_address(tmp, "any"):
            err_msg = ("Bad destination address")
            raise UFWError(err_msg)
        self.dst = tmp
        self._fix_anywhere()
    ...
    def set_interface(self, if_type, name):
        ...
        if if_type != "in" and if_type != "out":
            err_msg = ("Bad interface type")
            raise UFWError(err_msg)

        # Separate a few of the invalid checks out so we can give a nice error
        if '!' in str(name):
            err_msg = ("Bad interface name: reserved character: '!'")
            raise UFWError(err_msg)

        if ':' in str(name):
            err_msg = ("Bad interface name: can't use interface aliases")
            raise UFWError(err_msg)

        if str(name) == "." or str(name) == "..":
            err_msg = ("Bad interface name: can't use '.' or '..'")
            raise UFWError(err_msg)

        if (len(str(name)) == 0):
            err_msg = ("Bad interface name: interface name is empty")
            raise UFWError(err_msg)

        if (len(str(name)) > 15):
            err_msg = ("Bad interface name: interface name too long")
            raise UFWError(err_msg)

        # We are going to limit this even further to avoid shell meta
        if not re.match(r'^[a-zA-Z0-9_\-\.\+,=%@]+$', str(name)):
            err_msg = ("Bad interface name")
            raise UFWError(err_msg)
    ...
    def set_position(self, num):
        ...
        if str(num) != "-1" and not re.match(r'^[0-9]+', str(num)):
            err_msg = ("Insert position '%s' is not a valid position") % (num)
            raise UFWError(err_msg)
    ...
    def set_logtype(self, logtype):
        ...
        if logtype.lower() == "log" or logtype.lower() == "log-all" or \
           logtype == "":
            ...
            err_msg = ("Invalid log type '%s'") % (logtype)
            raise UFWError(err_msg)
    ...
    def set_direction(self, direction):
        ...
        if direction == "in" or direction == "out":
            ...
            err_msg = ("Unsupported direction '%s'") % (direction)
            raise UFWError(err_msg)
    ...
    def normalize(self):
        ...
        if self.src:
            try:
                ...
                err_msg = ("Could not normalize source address")
                raise UFWError(err_msg)
        ...
        if self.dst:
            try:
                ...
                err_msg = ("Could not normalize destination address")
                raise UFWError(err_msg)
    ...
    def match(x, y):
        ...
        if x.action == y.action and x.logtype == y.logtype and \
                x.comment == y.comment:
            dbg_msg = ("Found exact match")
            debug(dbg_msg)
            return 0
        if x.action == y.action and x.logtype == y.logtype and \
                x.comment != y.comment:
            dbg_msg = ("Found exact match, excepting comment")
            debug(dbg_msg)
            return -2

        dbg_msg = ("Found non-action/non-logtype/comment match " \
    ...
    def verify(self, rule_iptype):
        ...
        if self.protocol != "any" and \
           (self.sapp != "" or self.dapp != ""):
            err_msg = ("Improper rule syntax ('%s' specified with app rule)") \
                        % (self.protocol)
            raise UFWError(err_msg)
