...
class UFWBackend:
    ...
    def __init__(self, name, dryrun, extra_files=None,
        ...
        try:
            ...
            err_msg = ("Couldn't determine iptables version")
    ...
    def _get_default_policy(self, primary="input", check_forward=False):
        ...
        if check_forward and primary == "forward":
            enabled = False
            err_msg = ("problem running sysctl")
    ...
    def _do_checks(self): # pragma: no coverage
        ...
        if not self.do_checks:
            err_msg = ("Checks disabled")
            warn(err_msg)
            return True

        # Not needed on Linux, but who knows the places we will go...
        if os.getuid() != os.geteuid():
            err_msg = ("ERROR: this script should not be SUID")
            raise UFWError(err_msg)
        if os.getgid() != os.getegid():
            err_msg = ("ERROR: this script should not be SGID")
            raise UFWError(err_msg)

        uid = os.getuid()
        if uid != 0:
            err_msg = ("You need to be root to run this script")
            raise UFWError(err_msg)
        ...
        if not os.path.isdir(self.files['apps']):
            warn_msg = ("'%s' does not exist") % (self.files['apps'])
            warn(warn_msg)
        else:
            pat = re.compile(r'^\.')
        ...
        for path in list(self.files.values()) + \
            ...
            while True:
                ...
                if statinfo.st_uid != 0 and not is_unpack_user and \
                    ...
                    warn_msg = ("uid is %(uid)s but '%(path)s' is owned by " \
        ...
        for f in self.files:
            if f != 'apps' and not os.path.isfile(self.files[f]):
                err_msg = ("'%(f)s' file '%(name)s' does not exist") % \
    ...
    def _get_defaults(self):
        ...
        for c in [ 'input', 'output', 'forward' ]:
            if 'default_%s_policy' % (c) not in self.defaults:
                err_msg = ("Missing policy for '%s'" % (c))
                raise UFWError(err_msg)
            p = self.defaults['default_%s_policy' % (c)]
            if p not in policies:
                err_msg = ("Invalid policy '%(policy)s' for '%(chain)s'" % \
                            ({'policy': p, 'chain': c}))
                raise UFWError(err_msg)
    ...
    def set_default_application_policy(self, policy):
        ...
        rstr = ("Default application policy changed to '%s'") % (policy)
    ...
    def get_app_rules_from_template(self, template):
        ...
        if len(rules) < 1:
            err_msg = ("No rules found for application profile")
            raise UFWError(err_msg)
    ...
    def update_app_rule(self, profile):
        ...
        if updated_profile:
            ...
            rstr += ("Rules updated for profile '%s'") % (profile)

            try:
                self._write_rules(False) # ipv4
                self._write_rules(True) # ipv6
            except Exception: # pragma: no coverage
                err_msg = ("Couldn't update application rules")
                raise UFWError(err_msg)
    ...
    def find_application_name(self, profile_name):
        ...
        if matches == 1:
            ...
            err_msg = ("Found multiple matches for '%s'. Please use exact profile name") % \
                        (profile_name)
        else:
            err_msg = ("Could not find a profile matching '%s'") % \
                        (profile_name)
        raise UFWError(err_msg)
