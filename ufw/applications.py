'''applications.py: iptables backend for ufw'''
#
...
def get_profiles(profiles_dir):
    ...
    if not os.path.isdir(profiles_dir):
        err_msg = ("Profiles directory does not exist")
    ...
    for f in files:
        ...
        try:
            ...
            warn_msg = ("Skipping '%s': couldn't stat") % (f)
            warn(warn_msg)
            continue

        if size > max_size:
            warn_msg = ("Skipping '%s': too big") % (f)
            warn(warn_msg)
            continue

        if total_size + size > max_size:
            warn_msg = ("Skipping '%s': too many files read already") % (f)
            warn(warn_msg)
            continue
        ...
        try:
            ...
            warn_msg = ("Skipping '%s': couldn't process") % (f)
            warn(warn_msg)
            continue
        ...
        for p in cdict.sections():
            if len(p) > 64:
                warn_msg = ("Skipping '%s': name too long") % (p)
                warn(warn_msg)
                continue

            if not valid_profile_name(p):
                warn_msg = ("Skipping '%s': invalid name") % (p)
                warn(warn_msg)
                continue

            try:
                ufw.util.get_services_proto(p)
                warn_msg = ("Skipping '%s': also in /etc/services") % (p)
                warn(warn_msg)
                continue
            except Exception:
                pass
            ...
            for key, value in cdict.items(p):
                if len(key) > 64:
                    warn_msg = ("Skipping '%s': field too long") % (p)
                    warn(warn_msg)
                    skip = True
                    break
                if len(value) > 1024:
                    warn_msg = ("Skipping '%(value)s': value too long for " \
                                 "'%(field)s'") % \
                                 ({'value': p, 'field': key})
                    warn(warn_msg)
                    skip = True
            ...
            if p in profiles:
                warn_msg = ("Duplicate profile '%s', using last found") % (p)
                warn(warn_msg)
...
def verify_profile(name, profile):
    ...
    for f in app_fields:
        if f not in profile:
            err_msg = ("Profile '%(fn)s' missing required field '%(f)s'") % \
                        ({'fn': name, 'f': f})

            raise UFWError(err_msg)
        elif not profile[f]:
            err_msg = ("Profile '%(fn)s' has empty required field '%(f)s'") \
                        % ({'fn': name, 'f': f})
            raise UFWError(err_msg)
    ...
    try:
        ...
        debug(e)
        err_msg = ("Invalid ports in profile '%s'") % (name)
        raise UFWError(err_msg)
