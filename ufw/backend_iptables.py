...
class UFWBackendIptables(ufw.backend.UFWBackend):
    ...
    def set_default_policy(self, policy, direction):
        ...
        if not self.dryrun:
            if policy != "allow" and policy != "deny" and policy != "reject":
                err_msg = ("Unsupported policy '%s'") % (policy)
                raise UFWError(err_msg)

            if direction != "incoming" and direction != "outgoing" and \
               direction != "routed":
                err_msg = ("Unsupported policy for direction '%s'") % \
                            (direction)
                raise UFWError(err_msg)
        ...
        rstr = ("Default %(direction)s policy changed to '%(policy)s'\n") % \
                 ({'direction': direction, 'policy': policy})
        rstr += ("(be sure to update your rules accordingly)")
    ...
    def get_running_raw(self, rules_type):
        ...
        if self.dryrun:
            out = "> " + ("Checking raw iptables\n")
            out += "> " + ("Checking raw ip6tables\n")
            return out
    ...
    def get_status(self, verbose=False, show_count=False):
        ...
        if verbose:
            ...
            policy_str = ("Default: %(in)s (incoming), " +
                           "%(out)s (outgoing), " +
                           "%(routed)s (routed)") \
            ...
            return ("Status: active%s") % (s)

    def stop_firewall(self):
        '''Stop the firewall'''
        if self.dryrun:
            msg("> " + ("running ufw-init"))
        else:
            args = []
            ...
            (rc, out) = cmd(args)
            if rc != 0:
                err_msg = ("problem running ufw-init\n%s" % out)
                raise UFWError(err_msg)
            ...
            if 'loglevel' not in self.defaults or \
               self.defaults['loglevel'] not in list(self.loglevels.keys()):
                ...
                try:
                    ...
                    err_msg = ("Could not set LOGLEVEL")
                    raise UFWError(err_msg)
            else:
                try:
                    self.update_logging(self.defaults['loglevel'])
                except Exception:
                    err_msg = ("Could not load logging rules")
                    raise UFWError(err_msg)
    ...
    def _read_rules(self):
        ...
        for f in rfns:
            try:
                ...
                err_msg = ("Couldn't open '%s' for reading") % (f)
                raise UFWError(err_msg)
            ...
            for orig_line in orig:
                ...
                if pat_tuple.match(line):
                    ...
                    if len(tmp) < 6 or len(tmp) > 9:
                        wmsg = ("Skipping malformed tuple (bad length): %s") \
                                 % (tupl)
                        warn(wmsg)
                        continue
                    else:
                        ...
                        if len(tmp) == 7 or len(tmp) == 9:
                            wmsg = ("Skipping malformed tuple (iface): %s") \
                                     % (tupl)
                        ...
                        try:
                            ...
                            warn_msg = ("Skipping malformed tuple: %s") % \
                                        (tupl)
                            warn(warn_msg)
                            continue
    ...
    def _write_rules(self, v6=False):
        ...
        if not os.access(rules_file, os.W_OK):
            err_msg = ("'%s' is not writable" % (rules_file))
            raise UFWError(err_msg)
        ...
        * reloading the user rules file if rule is modified
        ...
        if rule.v6:
            if not self.use_ipv6():
                err_msg = ("Adding IPv6 rule failed: IPv6 not enabled")
                raise UFWError(err_msg)
            if rule.action == 'limit' and not self.caps['limit']['6']:
                # Rate limiting is runtime supported
                return ("Skipping unsupported IPv6 '%s' rule") % (rule.action)
        else:
            if rule.action == 'limit' and not self.caps['limit']['4']:
                # Rate limiting is runtime supported
                return ("Skipping unsupported IPv4 '%s' rule") % (rule.action)

        if rule.multi and rule.protocol != "udp" and rule.protocol != "tcp":
            err_msg = ("Must specify 'tcp' or 'udp' with multiple ports")
            raise UFWError(err_msg)
        ...
        if rule.v6:
            if self.iptables_version < "1.4" and (rule.dapp != "" or \
                                                  rule.sapp != ""):
                return ("Skipping IPv6 application rule. Need at least iptables 1.4")
            rules = self.rules6

        # bail if we have a bad position
        if position < 0 or position > len(rules):
            err_msg = ("Invalid position '%d'") % (position)
            raise UFWError(err_msg)

        if position > 0 and rule.remove:
            err_msg = ("Cannot specify insert and delete")
            raise UFWError(err_msg)
        if position > len(rules):
            err_msg = ("Cannot insert rule at position '%d'") % position
            raise UFWError(err_msg)
        ...
        if inserted:
            if matches > 0:
                rstr = ("Skipping inserting existing rule")
                if rule.v6:
                    rstr += " (v6)"
                return rstr
        else:
            ...
            if not found and rule.remove and not self.dryrun:
                rstr = ("Could not delete non-existent rule")
                if rule.v6:
                    rstr += " (v6)"
                return rstr
            elif found and not rule.remove and not modified:
                rstr = ("Skipping adding existing rule")
                if rule.v6:
                    rstr += " (v6)"
                return rstr
        ...
        try:
            ...
            err_msg = ("Couldn't update rules file")
            UFWError(err_msg)
        ...
        if self.is_enabled() and not self.dryrun:
            ...
            if modified or self._need_reload(rule.v6) or inserted:
                ...
                if allow_reload:
                    ...
                    rstr += (" (skipped reloading firewall)")
    ...
    def _chain_cmd(self, chain, args, fail_ok=False):
        ...
        (rc, out) = cmd([exe] + args)
        if rc != 0:
            err_msg = ("Could not perform '%s'" % (args))
            if fail_ok:
    ...
    def update_logging(self, level):
        ...
        try:
            ...
            err_msg = ("Couldn't update rules file for logging")
            UFWError(err_msg)
    ...
    def _get_logging_rules(self, level):
        ...
        if level not in list(self.loglevels.keys()):
            err_msg = ("Invalid log level '%s'") % (level)
            raise UFWError(err_msg)
    ...
    def reset(self):
        ...
        for i in self.files:
            ...
            if not os.path.isfile(fn):
                err_msg = ("Could not find '%s'. Aborting") % (fn)
                raise UFWError(err_msg)
        ...
        for i in allfiles:
            ...
            if os.path.exists(fn):
                err_msg = ("'%s' already exists. Aborting") % (fn)
                raise UFWError(err_msg)

        # Move the old to the new
        for i in allfiles:
            fn = "%s.%s" % (i, ext)
            res += ("Backing up '%(old)s' to '%(new)s'\n") % (\
        ...
        for i in allfiles:
            ...
            try:
                ...
                warn_msg = ("Couldn't stat '%s'") % (i)
                warn(warn_msg)
                continue

            if mode & stat.S_IWOTH:
                res += ("WARN: '%s' is world writable") % (i)
            elif mode & stat.S_IROTH:
                res += ("WARN: '%s' is world readable") % (i)
