...
def parse_port_proto(p_str):
    ...
    if len(tmp) == 1:
        ...
        if proto in portless_protocols:
            err_msg = ("Invalid port with protocol '%s'" % proto)
...
def under_ssh(pid=os.getpid()):
    ...
    try:
        ...
        warn_msg = ("Couldn't find pid (is /proc mounted?)")
        warn(warn_msg)
        return False
    except Exception:
        err_msg = ("Couldn't find parent pid for '%s'") % (str(pid))
        raise ValueError(err_msg)
    ...
    if not os.path.isfile(path): # pragma: no cover
        err_msg = ("Couldn't find '%s'") % (path)
        raise ValueError(err_msg)

    try:
        exe = open(path).readlines()[0].split()[1]
    except Exception: # pragma: no cover
        err_msg = ("Could not find executable for '%s'") % (path)
        raise ValueError(err_msg)
...
def get_netstat_output(v6):
    ...
    for p in proto:
        try:
            ...
            warn_msg = ("Could not get statistics for '%s'" % (p))
            warn(warn_msg)
            continue
