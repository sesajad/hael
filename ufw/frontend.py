...
class UFWFrontend:
    ...
    def set_enabled(self, enabled):
        ...
        if enabled:
            ...
            res = ("Firewall is active and enabled on system startup")
        else:
            try:
            ...
            res = ("Firewall stopped and disabled on system startup")
    ...
    def get_show_listening(self):
        ...
        try:
            ...
            err_msg = ("Could not get listening status")
    ...
    def set_rule(self, rule, ip_version):
        ...
        for i, r in enumerate(rules):
            ...
            try:
                if self.backend.use_ipv6():
                    ...
                    if ip_version == "v4" or ip_version == "both":
                        ...
                        err_msg = ("IPv6 support not enabled")
                        raise UFWError(err_msg)
                    else:
                        err_msg = ("Invalid IP version '%s'") % (ip_version)
                        raise UFWError(err_msg)
            ...
            if r.updated:
                warn_msg = ("Rule changed after normalization")
            ...
            for j in indexes:
                if count > 0 and rules[j]:
                    ...
                    try:
                        ...
                        undo_error = True
                        warn_msg = ("Could not back out rule '%s'") % \
                                     r.format_rule()
                        warn(warn_msg)

            err_msg += ("\nError applying application rules.")
            if undo_error:
                err_msg += (" Some rules could not be unapplied.")
            else:
                err_msg += (" Attempted rules successfully unapplied.")
    ...
    def delete_rule(self, number, force=False):
        ...
        try:
            ...
            err_msg = ("Could not find rule '%s'") % number
            raise UFWError(err_msg)

        rules = self.backend.get_rules()
        if n <= 0 or n > len(rules):
            err_msg = ("Could not find rule '%d'") % n
            raise UFWError(err_msg)
    ...
    def do_action(self, action, rule, ip_version, force=False):
        ...
        if action.startswith("logging-on"):
            ...
            err_msg = ("Unsupported default policy")
            tmp = action.split('-')
            if len(tmp) != 3:
                raise UFWError(err_msg)
            ...
            if self.backend.is_enabled():
                ...
                res = ("Firewall reloaded")
            else:
                res = ("Firewall not enabled (skipping reload)")
            ...
            err_msg = ("Unsupported action '%s'") % (action)
            raise UFWError(err_msg)
    ...
    def application_add(self, profile):
        ...
        if profile == "all":
            err_msg = ("Cannot specify 'all' with '--add-new'")
            raise UFWError(err_msg)
        ...
        if default == "skip":
            ...
            err_msg = ("Unknown policy '%s'") % (default)
            raise UFWError(err_msg)
    ...
    def continue_under_ssh(self):
        ...
        if self.backend.do_checks and ufw.util.under_ssh(): # pragma: no cover
            prompt = ("Command may disrupt existing ssh connections. " \
    ...
    def reset(self, force=False):
        '''Reset the firewall'''
        res = ""
        prompt = ("Resetting all rules to installed defaults. Proceed with " \
                   "operation (%(yes)s|%(no)s)? ") % \
                   ({'yes': self.yes, 'no': self.no})
        if self.backend.do_checks and ufw.util.under_ssh():
            prompt = ("Resetting all rules to installed defaults. This may " \
